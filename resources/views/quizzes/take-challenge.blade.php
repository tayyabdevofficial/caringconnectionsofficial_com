...
@section('title', 'How Well Do You Know ' . ($challenge['creator_name'] ?? 'Your Loved One') . '? - Caring Connections Dare')
@section('meta_description', ($challenge['creator_name'] ?? 'Your loved one') . ' has invited you to guess their answers on ' . ($quiz['title'] ?? 'this connection quiz') . '! Can you achieve a 100% soul match?')
        ...
        <div class="bg-gradient-to-br from-[#281858] via-[#432379] to-[#8B5CF6] rounded-3xl text-white p-6 sm:p-10 shadow-2xl text-center relative overflow-hidden">
            ...
            <div class="relative z-10 space-y-4">
                ...
                <div class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/20 backdrop-blur-md text-xs font-bold uppercase tracking-wider text-purple-200">
                    <span>✨</span> Caring Dare Accepted!
                </div>

                <h1 class="text-2xl sm:text-4xl font-black font-serif tracking-tight">
                    How Deeply Do You Know <span class="underline decoration-purple-300 decoration-wavy">{{ $challenge['creator_name'] }}</span>?
                </h1>

                <p class="text-sm sm:text-base text-purple-100/90 max-w-lg mx-auto">
                    {{ $challenge['creator_name'] }} has personally answered these heartfelt questions. Guess their genuine choices and see your bond ranking!
                </p>
            </div>
        </div>

        <!-- Progress Tracker -->
        <div class="bg-white dark:bg-[#140C26] rounded-2xl border border-purple-100 dark:border-purple-900/30 p-4 shadow-sm">
            ...
            <div class="w-full h-2.5 bg-purple-100 dark:bg-purple-950/60 rounded-full overflow-hidden">
                <div id="friend-progress-bar" class="h-full bg-gradient-to-r from-[#8B5CF6] via-purple-600 to-rose-500 transition-all duration-300 w-0 rounded-full"></div>
            </div>
        </div>
        ...
        <form id="friend-dare-form" action="{{ route('quizzes.challenge.submit', $challenge['share_token']) }}" method="POST" class="space-y-8">
            ...
            <div class="bg-white dark:bg-[#140C26] rounded-3xl border border-purple-100 dark:border-purple-900/30 p-6 sm:p-8 shadow-sm space-y-4">
                ...
                <p class="text-xs text-slate-500 dark:text-slate-400">
                    Your score and match percentage will appear on {{ $challenge['creator_name'] }}'s scoreboard.
                </p>
            </div>
            ...
            <div class="sticky bottom-4 z-30 bg-white/95 dark:bg-[#140C26]/95 backdrop-blur-md rounded-3xl border border-purple-100 dark:border-purple-900/40 p-4 sm:p-6 shadow-2xl flex flex-col sm:flex-row items-center justify-between gap-4">
                <div>
                    <div class="font-bold text-slate-900 dark:text-white text-base font-serif">Finished your guesses?</div>
                    <div class="text-xs text-slate-500 dark:text-slate-400">Discover your compatibility &amp; score immediately!</div>
                </div>
                <button type="submit" id="submit-friend-dare-btn" class="w-full sm:w-auto px-8 py-4 rounded-2xl bg-gradient-to-r from-[#8B5CF6] via-purple-700 to-rose-500 text-white font-bold text-base shadow-lg hover:shadow-xl hover:scale-105 active:scale-95 transition-all duration-200 flex items-center justify-center gap-2">
            ...
            <div class="bg-white dark:bg-[#140C26] rounded-3xl border border-purple-100 dark:border-purple-900/30 p-6 sm:p-8 shadow-sm space-y-4">
                <div class="flex items-center justify-between border-b border-purple-50 dark:border-purple-900/30 pb-3">
                    <h3 class="font-bold text-base text-slate-900 dark:text-white flex items-center gap-2 font-serif">
                        <span>🏆</span> Current Top Loved Ones
                    </h3>
                    ...
                    @foreach(array_slice($leaderboard, 0, 5) as $i => $item)
                        <div class="py-2.5 flex items-center justify-between text-xs sm:text-sm">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <span class="px-2 py-0.5 rounded-full text-xs font-bold bg-purple-100 dark:bg-purple-950/60 text-[#8B5CF6] dark:text-purple-300">{{ $item['percentage'] }}%</span>
                            </div>
                        </div>
                    @endforeach
                </div>
