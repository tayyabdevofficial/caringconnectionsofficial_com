...
class QuizController extends Controller
{
    ...
    public function index()
    {
        ...
        if (isset($data['quizzes_enabled']) && !$data['quizzes_enabled']) {
            ...
            return response()->view('quizzes.disabled', array_merge($common, [
                'quizTitle' => 'Community Challenges',
                'message' => 'The quiz hub is currently undergoing scheduled maintenance. Please check back soon!',
                'otherQuizzes' => [],
                'siteTitle' => 'Quizzes Under Maintenance - ' . config('site.name'),
            ]), 200);
        }
        ...
        return view('quizzes.index', array_merge($common, [
            ...
            'siteTitle' => 'Empathy & Friendship Challenges - ' . config('site.name'),
            'siteDescription' => 'Create thoughtful personal friendship challenges, share with loved ones, and celebrate your connections!',
        ]));
    }
    ...
    public function createChallenge(string $slug)
    {
        ...
        if (!empty($data['disabled']) || (isset($data['quiz']['status']) && !$data['quiz']['status'])) {
            ...
            return response()->view('quizzes.disabled', array_merge($common, [
                ...
                'message' => 'This challenge has been paused by the admin. Don\'t worry, there are other meaningful challenges to explore!',
                'otherQuizzes' => $quizzesData['quizzes'] ?? [],
                'siteTitle' => 'Challenge Inactive - ' . config('site.name'),
            ]), 200);
        }
        ...
        if (!$quiz) {
            abort(404, 'Challenge not found or currently unavailable.');
        }
        ...
        return view('quizzes.create-challenge', array_merge($common, [
            'quiz' => $quiz,
            'siteTitle' => 'Create Challenge: ' . ($quiz['title'] ?? 'Challenge') . ' - ' . config('site.name'),
            'siteDescription' => 'Answer questions thoughtfully, generate your unique link, and share with your loved ones!',
        ]));
    }
    ...
    public function storeChallenge(Request $request, string $slug)
    {
        ...
        return redirect()->route('quizzes.challenge.share', $token)->with('success', 'Your challenge is live! Share it with your friends and family below.');
    }
    ...
    public function shareDashboard(string $token)
    {
        ...
        return view('quizzes.challenge-share', array_merge($common, [
            ...
            'siteTitle' => ($res['challenge']['creator_name'] ?? 'Friend') . "'s Challenge - Share & Scoreboard",
        ]));
    }
    ...
    public function takeChallenge(Request $request, string $token)
    {
        ...
        // If the user already submitted answers for this challenge, redirect to score page
        ...
        return view('quizzes.take-challenge', array_merge($common, [
            ...
            'quiz' => $res['quiz'],
            'leaderboard' => $res['leaderboard'] ?? [],
            'siteTitle' => 'How Well Do You Know ' . ($res['challenge']['creator_name'] ?? 'Your Friend') . '? - ' . config('site.name'),
            'siteDescription' => 'Accept the challenge! Guess their answers and see how high you score.',
        ]));
    }
    ...
    public function submitAttempt(Request $request, string $token)
    {
        ...
        if (session()->has('quiz_attempt_' . $token) || $request->hasCookie('quiz_attempt_' . $token)) {
            return redirect()->route('quizzes.challenge.score', $token)->with('info', 'You have already submitted your answers for this challenge!');
        }
        ...
        cookie()->queue('quiz_attempt_' . $token, json_encode([
            ...
            'verdict' => $result['verdict'] ?? 'Challenge Complete',
    ...
    public function showScore(Request $request, string $token)
    {
        ...
        return view('quizzes.challenge-result', array_merge($common, [
            'result' => $result,
            'token' => $token,
            'siteTitle' => ($result['friend_name'] ?? 'You') . ' scored ' . ($result['percentage'] ?? 0) . '% on ' . ($result['creator_name'] ?? 'Friend') . "'s Challenge!",
        ]));
    }
}
